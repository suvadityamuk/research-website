...
You want to learn how to use `ffmpeg` for the same reason you don't want to use a ChatGPT or Gemini wrapper. Because you inherently are looking for the speed benefit that wrappers tend to eat up as overhead. Because you are working with a huge amount of data and a per-sample-improvement of 0.1% can mean the difference between a successful and failed project.
